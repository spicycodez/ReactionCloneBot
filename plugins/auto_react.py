...
async def auto_react_handler(client: Client, message: Message):
    ...
    try:
        ...
        FALLBACK_EMOJI = "👍"  # near-universally allowed, used when the chosen emoji is rejected
        ...
        try:
            try:
                ...
                err = str(e)
                if "REACTIONS_TOO_MANY" in err and len(chosen) > 1:
                    # This chat doesn't allow multiple reaction types at once
                    # (that's a per-chat Telegram setting, off by default) -
                    # fall back to sending just one emoji instead.
                    await send_bot_api_reaction(client, chat_id, message.id, chosen[:1])
                elif "REACTION_INVALID" in err and chosen != [FALLBACK_EMOJI]:
                    # The chosen emoji isn't in this specific chat's currently
                    # allowed-reactions set (a per-chat Telegram setting).
                    # Rather than silently skipping the reaction on this
                    # message, retry once with a near-universally-allowed
                    # emoji so the message still gets a reaction.
                    logger.warning(
                        "Emoji %s not allowed by Telegram in chat %s, falling back to %s",
                        chosen, chat_id, FALLBACK_EMOJI
                    )
                    await send_bot_api_reaction(client, chat_id, message.id, [FALLBACK_EMOJI])
                else:
                    raise
            ...
            elif any(x in err for x in ("MESSAGE_ID_INVALID", "message to react not found", "CHAT_NOT_FOUND")) \
