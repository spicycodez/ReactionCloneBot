...
/listowners - Sab owners dekho
"""

HELP_TEXT_SUPER_OWNER = """
**Super Owner Commands:**
/addowner <user_id> - Kisi ko bhi owner access do (bina redeploy)
/removeowner <user_id> - Kisi added owner ka access hatao
"""
...
async def help_cmd(client: Client, message: Message):
    ...
    if is_master and uid is not None:
        is_super_owner = uid in Config.OWNER_IDS
        if is_super_owner or await owner_db.is_owner(uid):
            text += HELP_TEXT_MASTER_OWNER
        if is_super_owner:
            text += HELP_TEXT_SUPER_OWNER
